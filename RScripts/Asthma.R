...
control <- rbind(obese_control, no_control)
asthma <- rbind(obese_asthma, no_asthma)
...
dat <- subset(dat, select = c(control$GEO_Accession, asthma$GEO_Accession))
...
write.csv(finalFrame, file="Asthma/Output/Asthma_Result.csv")
