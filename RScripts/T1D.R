...
dat <- read.csv("T1D/Data/T1D_MA.csv", header=TRUE, stringsAsFactors=FALSE)
meta <- read.csv("T1D/Data/T1D_Meta.csv", header = TRUE, stringsAsFactors=FALSE)
meta$Status = c(replicate(10,"Control"), replicate(12, "T1D"))

control <- meta[meta$Status == "Control", ]
t1d <- meta[meta$Status == "T1D", ]
...
dat <- subset(dat, select = c(t1d$X.Sample_geo_accession, control$X.Sample_geo_accession))
...
write.csv(finalFrame, file="T1D/Output/T1D_Result.csv")
